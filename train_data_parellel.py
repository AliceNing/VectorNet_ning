...
def train_model(epochs, train_loader, eval_loader, vector_net, optimizer, is_print_eval=True,  learning_rate=0.001, decayed_factor=0.3):
        ...
        for i, data in enumerate(train_loader):
            ...
            if is_print_eval:
                ...
                for i, data in enumerate(eval_loader):
                    inputs, labels = data
                    outputs = vector_net(data["item_num"].to(config.device),  data["polyline_list"])
                    loss += loss_func(outputs, data["gt_preds"].to(config.device))
                    ade += torch.mean(get_ADE(outputs, data["gt_preds"].to(config.device)))
                    t += 1
                if t > 0:
                    loss /= t
                    ade /= t
